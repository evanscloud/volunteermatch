...
RSpec.describe Volunteermatch::API::SearchOrganizations do
  ...
  describe "searchOrganizations api call" do
    ...
    it "should call #search_organizations with appropriate arguments" do
      expect(subject).to receive(:search_organizations).with({location: "94108", nbOfResults: 10, pageNumber: 3, fieldsToDisplay: ["name", "location"], names: ["red cross"]})
      subject.search_organizations(location: "94108", nbOfResults: 10, pageNumber: 3, fieldsToDisplay: ["name", "location"], names: ["red cross"])
    end

    it "returns a list of organizations requested" do
      url = URI.parse("http://www.volunteermatch.org/api/call?action=searchOrganizations&query=" + URI.encode({ location: "94108", nbOfResults: 10, pageNumber: 3, fieldsToDisplay: ["name", "location"], names: ["red cross"] }.to_json))
